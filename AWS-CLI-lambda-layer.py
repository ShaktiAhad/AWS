...
os.system(
    '''
    if [[ $(python3 --version) && $(pip3 --version) && $(virtualenv --version) != "command not found" ]];
    then
        ...
        if [ $(python3 --version)  != "command not found" ];
        then
            echo "python3 is installed on your machine": $(python3 --version)
        else
            echo "python3 is not installed on your machine. Download url for Mac: '"https://www.python.org/downloads/macos/"'"
        fi
        ...
        if [ $(virtualenv --version)  != "command not found" ];
        then
            echo "virtualenv is installed on your machine": $(python3 --version)
        else
            echo "virtualenv is not installed on your machine use '"pip3 install virtualenv"'"
        fi 
    fi
